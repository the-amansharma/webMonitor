...
def load_data():
    with lock:
        try:
            ...
            print(f"⚠️ Corrupted JSON detected, moved to {backup_file}")
            with open(DATA_FILE, "w") as f:
                json.dump([], f, indent=2)
            return []
...
def perform_site_check(site):
    ...
    update_stats(site)
    trigger_notifications(site)  # Toast & email


# ------------------ Notification Helpers ------------------
def trigger_notifications(site):
    """
    Placeholder for:
    - Broadcasting toast messages (SSE/WebSocket)
    - Sending emails for sites with notifications enabled
    """
    # Example: broadcast_site_event(site, "manual_check")
    if site.get("notifications_enabled") and site.get("notification_email"):
        send_email(site["notification_email"], f"Site Alert: {site['name'] or site['url']}",
                   f"{site['name'] or site['url']} status: {site['status']}")
...
def delete_website(site_id):
    try:
        ...
        save_data(new_sites)
        # TODO: broadcast_site_event to notify others
...
# ------------------ Manual Check Route (with per-site lock) ------------------
...
def manual_check(site_id):
    try:
        ...
        # Create per-site lock if not exists
        ...
        if not lock_obj.acquire(blocking=False):
            return jsonify({"error": f"A manual check for {site['url']} is already running. Please wait."}), 429
